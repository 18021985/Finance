...
class EnsemblePredictor:
    ...
    def prepare_features(self, hist: pd.DataFrame) -> np.ndarray:
        ...
        atr = float(true_range.rolling(14).mean().iloc[-1]) if not true_range.empty else 0.0
        features.append(atr)
        
        # OBV (On-Balance Volume)
        if 'Volume' in hist.columns:
            obv = (np.sign(hist['Close'].diff()) * hist['Volume']).cumsum()
            obv_change = float((obv.iloc[-1] / obv.iloc[-20:].mean() - 1) * 100) if len(obv) > 20 else 0.0
        else:
            obv_change = 0.0
        ...
        features.append(float(cci.iloc[-1]) if not cci.empty else 0.0)
        
        # Williams %R
        high_14 = hist['High'].rolling(14).max()
        low_14 = hist['Low'].rolling(14).min()
        williams_r = -100 * (high_14 - hist['Close']) / (high_14 - low_14)
        features.append(float(williams_r.iloc[-1]) if not williams_r.empty else 0.0)
        
        # Stochastic %K
        low_14 = hist['Low'].rolling(14).min()
        high_14 = hist['High'].rolling(14).max()
        stoch_k = 100 * (hist['Close'] - low_14) / (high_14 - low_14)
        features.append(float(stoch_k.iloc[-1]) if not stoch_k.empty else 0.0)
